...
### 2. 古典概率模型
球与桶模型：有𝑛个球，每个球都等可能被放到𝑚个桶中的任一个。求每个桶中至多有一个球的概率。
...
$$
作业一第三题：证明$P_{n,m} \geq e^{-\frac{{n-1}n}{2m}}(1-\frac{8n^3}{m^2})$
...
- 非负性：$P(A) \in [0, 1]$
- 规范性：$P(S) = 1$
- 有限可加性：对互不相容事件$A_i$有$P(\bigcup_{i} A_i) = \sum_{i} P(A_i)$

### 4. 概率的公理化
- 非负性：$\forall A \in F, P(A) \geq 0$
- 规范性：$P(S) = 1$
- 可列可加性：对互不相容事件$A_1, A_2, A_3, ...$有$P(\bigcup_{i}^{+\infty}A_i) = \sum_{i=1}^{+\infty}P(A_i)$
...
反例：$S=[0, \infty), P(A) = \lim_{k \to \infty}\frac{1}{k} \lambda(A \cap (0,k)), A_i = [i-1, i)$，其中$\lambda(A)$为$A$的长度。
