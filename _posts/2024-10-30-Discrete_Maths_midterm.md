...
### 1. 基本概念
- 幂集
  - **\\(P(A), 2^A\\)：设\\(A\\)为集合，称\\(A\\)的全体子集构成的集合为\\(A\\)的幂集**
  - \\(P(A)={X \mid X \\subseteq A}\\)
  - 如果\\(A\\)是\\(n\\)元集，\\(P(A)\\)有\\(2^n\\)个元素
- 对称差
  - **\\(A \oplus B = (A-B)\cup (B-A)\\)**
- 广义并
  - **\\(\bigcup \mathcal{A} \\)：设\\(\mathcal{A} \\)为一个集族，\\(\mathcal{A} \\)中全体元素的元素组成的集合称为\\(\mathcal{A} \\)的广义并**
  - \\(\mathcal{A} = {x \mid \exists A in \mathcal{A} s.t. x in A}\\)
  - \\(\bigcup \varnothing  = \varnothing \\)
- 广义交
  - **\\(\bigcap \mathcal{A}\\)：设\\(\mathcal{A} \\)为一个集族，\\(\mathcal{A} \\)中全体元素的公共元素组成的集合称为\\(\mathcal{A} \\)的广义交**
